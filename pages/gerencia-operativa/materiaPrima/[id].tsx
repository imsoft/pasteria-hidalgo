...
export const MateriaPrimaPage: FC<Props> = ({ materiaPrima }) => {
  ...
  const { actualizarMateriaPrima, eliminarMateriaPrima } =
    useContext(MateriasPrimasContext);

  const [inputMateriaPrima, setInputMateriaPrima] = useState(materiaPrima.materiaPrima);
  const [inputUnidades, setInputUnidades] = useState<Unidades>(materiaPrima.unidades);
  const [inputTemperatura, setInputTemperatura] = useState<Temperatura>(materiaPrima.temperatura);
