...
export default async function handler(
  ...
  await CheckInDePersonal.insertMany(seedDatabase.initialData.checkInDePersonal);

  await AcondicionamientoDeSucursal.deleteMany(); //Borra todo de la DB
  await AcondicionamientoDeSucursal.insertMany(seedDatabase.initialData.acondicionamientoDeSucursal);
  ...
  await ReporteVentasAmbulantesIndividual.insertMany(seedDatabase.initialData.reporteVentasAmbulantesIndividual);

  await ReporteVentasIndividual.deleteMany(); //Borra todo de la DB
  await ReporteVentasIndividual.insertMany(seedDatabase.initialData.reporteVentasIndividual);
  ...
  await SucursalYFranquicia.insertMany(seedDatabase.initialData.sucursalYFranquicia);

  await PersonalDeMantenimiento.deleteMany(); //Borra todo de la DB
  await PersonalDeMantenimiento.insertMany(seedDatabase.initialData.personalDeMantenimiento);
  ...
  await ReporteDeGanancia.deleteMany(); //Borra todo de la DB
  // await ReporteDeGanancia.insertMany(seedDatabase.initialData.reporteDeGanancia);
